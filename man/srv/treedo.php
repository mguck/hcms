...
switch($_POST["tpl"]){     
  case "addnod":#添加节点
  	$nid=$pd->genid($tbl);
  	$pid=$_POST["pid"];
  	$odx=empty($_POST["odx"])?0:$_POST["odx"];
    $visible=$_POST["visible"];
  	if(empty($_POST["pid"])){
      $sql="insert into `".$tbl."`(id,pid,name,childnums,odx,visible,timestamp,nums".$field.") values(".$nid.",".$pid.",'".$_POST["name"]."',0,".$odx.",".$visible.",UNIX_TIMESTAMP(),0".$type.")";
  	}else{	
  		$sql="insert into `".$tbl."`(id,pid,name,childnums,odx,visible,timestamp,nums".$field.") values(".$nid.",".$pid.",'".$_POST["name"]."',0,".$odx.",".$visible.",UNIX_TIMESTAMP(),0".$type.")";	
    }  
    $pd->exec($sql);
  ...
  case "delnod":
    ...
    $pd->exec("delete from `".$tbl."` where id=".$id); 
  	$nums=$pd->query("select count(1) from `".$tbl."` where pid=".$pid)->fetchColumn(0); 
  	$pd->exec("update `".$tbl."` set childnums=".$nums." where id=".$pid);
      echo "ok";
      break;
