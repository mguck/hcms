...
function addTreeNode(d){	
	...
	if(node==undefined){alert("请选择一个分类1");return;}
	if(!node||node==null){alert("请选择一个分类2");return;}		
	$(d.dlg).data("treeid",d.tree);	
...
function addTreeNodeDo(id){
   ...
   /*   t.tree('append', {
    ...
    saveToDb({tpl:"addnod",pid:$(id).data("pid"),name:$(id+" #name").val(),odx:$(id+" #odx").val(),visible:$(id+" #visible").val(),dlg:id});
	  $(id).modal('hide');
}
